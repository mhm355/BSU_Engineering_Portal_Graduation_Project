# ECS Cluster and Task Definitions for Fargate
...
# Task Definition for Frontend (Nginx serving React build)
...
# Task Definition for Backend (Django/Gunicorn with Redis sidecar)
resource "aws_ecs_task_definition" "backend" {
  family                   = "${var.project_name}-backend"
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  cpu                      = var.backend_cpu + 256  # Extra CPU for Redis
  memory                   = var.backend_memory + 512 # Extra memory for Redis
  execution_role_arn       = aws_iam_role.ecs_task_execution.arn
  task_role_arn            = aws_iam_role.ecs_task.arn
...
# ECS Service Auto Scaling for Frontend
