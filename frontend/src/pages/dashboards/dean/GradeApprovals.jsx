...
import {
    Container, Typography, Box, Paper, Button, Chip, CircularProgress, Alert, Dialog, DialogTitle,
    DialogContent, DialogActions, Grid, Card, CardContent, Avatar, Fade, Grow
} from '@mui/material';
...
const StatCard = ({ icon: Icon, value, label, color, delay = 0 }) => (
    <Grow in={true} timeout={800 + delay}>
        <Paper elevation={0} sx={{ p: 2.5, borderRadius: 3, background: '#fff', boxShadow: '0 4px 20px rgba(0,0,0,0.08)', border: '1px solid rgba(0,0,0,0.06)', transition: 'all 0.3s ease', '&:hover': { transform: 'translateY(-5px)', boxShadow: `0 20px 40px ${color}25` } }}>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                <Box sx={{ width: 50, height: 50, borderRadius: 3, background: `linear-gradient(135deg, ${color}, ${color}cc)`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 8px 24px ${color}40` }}>
                    <Icon sx={{ fontSize: 24, color: '#fff' }} />
                </Box>
                <Box>
                    <Typography variant="h5" sx={{ fontWeight: 'bold', color: '#1a2744', fontFamily: 'Cairo', lineHeight: 1 }}>{value}</Typography>
...
export default function GradeApprovals() {
    ...
    useEffect(() => { fetchPendingGrades(); }, []);

    const fetchPendingGrades = async () => {
        try {
            const res = await axios.get('/api/academic/exam-grades/pending/', config);
            setPendingGrades(Array.isArray(res.data) ? res.data : (res.data.results || []));
    ...
    return (
        <Box sx={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f5f7fa 0%, #e4e8ec 100%)', pb: 6 }}>
            {/* Header */}
            <Box sx={{ background: 'linear-gradient(135deg, #FFD93D 0%, #FF9800 100%)', pt: 3, pb: 4, mb: 3, position: 'relative', overflow: 'hidden' }}>
                <Container maxWidth="xl">
                    <Fade in={true} timeout={800}>
                        <Box>
                            <Button startIcon={<ArrowBackIcon />} onClick={() => navigate('/dean/dashboard')} sx={{ color: '#1a2744', mb: 2, fontFamily: 'Cairo', '&:hover': { bgcolor: 'rgba(255,255,255,0.2)' } }}>
                                العودة للوحة التحكم
                            </Button>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                                <Avatar sx={{ width: 55, height: 55, bgcolor: 'rgba(255,255,255,0.3)', backdropFilter: 'blur(10px)' }}>
                                    <PendingActionsIcon sx={{ fontSize: 30, color: '#1a2744' }} />
                                </Avatar>
                                <Box>
                                    <Typography variant="h5" sx={{ fontFamily: 'Cairo', fontWeight: 'bold', color: '#1a2744' }}>إعتماد الدرجات</Typography>
                                    <Typography variant="subtitle1" sx={{ fontFamily: 'Cairo', color: 'rgba(26,39,68,0.8)' }}>مراجعة واعتماد درجات الطلاب النهائية</Typography>
                                </Box>
                            </Box>
                        </Box>
                    </Fade>
            ...
            <Container maxWidth="xl">
                ...
                <Grid container spacing={3} sx={{ mb: 4 }}>
                    <Grid item xs={12} sm={6} md={4}><StatCard icon={PendingActionsIcon} value={pendingGrades.length} label="طلبات درجات معلقة" color="#FF9800" delay={0} /></Grid>
                    <Grid item xs={12} sm={6} md={4}><StatCard icon={GradingIcon} value={totalMidterm + totalFinal} label="إجمالي الدرجات" color="#9C27B0" delay={100} /></Grid>
                    <Grid item xs={12} sm={12} md={4}><StatCard icon={WarningAmberIcon} value={totalMidterm} label="درجات منتصف الترم" color="#2196F3" delay={200} /></Grid>
                </Grid>
