...
# Create superuser if it doesn't exist, and always sync password from env
...
admin_password = os.environ.get('ADMIN_PASSWORD', 'admin123@bsu')
admin.set_password(admin_password)
if created:
    admin.first_login_required = True
admin.save()
if created:
    print('Admin user created with password from environment.')
else:
    print('Admin user exists - password synced from environment.')
