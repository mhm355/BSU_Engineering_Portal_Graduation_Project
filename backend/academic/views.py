...
class CourseOfferingViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=True, methods=['get'], permission_classes=[IsDoctorRole])
    def students(self, request, pk=None):
        """Get students for a specific course offering (for Doctor)"""
        offering = self.get_object()
        
        # Verify doctor owns this course
        if offering.doctor != request.user:
            return Response(
                {'error': 'غير مصرح لك بالوصول لهذا المقرر'},
                status=status.HTTP_403_FORBIDDEN
            )
        
        # Get students in this level
        students = Student.objects.filter(level=offering.level).select_related(
            'level', 'department', 'academic_year'
        )
        ...
        for student in students:
            result.append({
                ...
                'university_id': student.national_id,
                'level_name': student.level.get_name_display() if student.level else None,
