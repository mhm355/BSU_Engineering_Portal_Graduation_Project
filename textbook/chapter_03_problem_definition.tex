...
\chapter{Problem Definition and Requirements Engineering}
\label{chap:requirements}
\newpage
\section*{Problem Definition and Requirements Engineering}


\section{System Scope and Limitations}
While the BSU Engineering Portal provides comprehensive Management Information System (MIS) capabilities for academic administration and alumni career tracking, traditional Learning Management System (LMS) features such as Discussion Forums and Live Virtual Classrooms are intentionally scoped for future phases. This distinguishes the current system from platforms like Moodle, focusing first on core administrative reliability and the complete student-to-alumni lifecycle before expanding into interactive pedagogical tools.

\section{Current Situation and Stakeholder Analysis}
...
\begin{itemize}
    ...
    \item \textbf{Alumni Disconnect}: Graduation clearance required students to physically carry paper signature sheets to the library, finance, and labs. Furthermore, after graduation, the university lost structured contact with alumni, severely hindering employment tracking and the distribution of official certificates.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Stakeholder Interaction Diagram showing all eight roles (Admin, Dean, HOD, Doctor, Student, Student Affairs, Staff Affairs, Graduate Affairs) and their primary data flows into the centralized system.}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.15\textwidth}|p{0.2\textwidth}|p{0.3\textwidth}|p{0.25\textwidth}|}
        ...
        \hline
        \textbf{Staff Affairs} & Admin staff managing HR & No structured system for managing doctor course assignments. & Batch-upload doctors, manage deletions. \\
        \hline
        \textbf{Graduate Affairs} & Admin staff managing Alumni & Paper-based graduation clearance and no career portal. & Manage clearance, jobs, events, and official certificates. \\
        \hline
        \textbf{Dean} & Executive Academic Head & Lack of visibility into grading pipelines and results. & Publish results, view analytics. \\
        \hline
        \textbf{HOD} & Head of Department & Disjointed assignment tracking. & Assign doctors to specific courses. \\
        \hline
        \textbf{Admin} & IT/System Administrator & No central configuration tool; ad-hoc management. & Manage academic years/terms, approve deletions, view audit logs. \\
        \hline
    \end{tabular}
...
The following code snippet demonstrates how these boundaries are strictly enforced at the API layer using Django REST Framework~\cite{djangorest} permission classes:
...
The BSU Engineering Portal was developed using an \textbf{iterative, sprint-based Agile methodology} [19]. A traditional Waterfall approach [18] would have mandated locking all requirements prior to implementation. However, stakeholder interviews frequently revealed new requirements throughout the development cycle. For example, the necessity for a multi-modal quiz engine (supporting MCQ, Essay, and Image-based questions) emerged mid-development when faculty expressed frustration over the lack of integrated online assessment tools.
...
\begin{enumerate}
    ...
    \item \textbf{Maintain continuous deployment}: Docker [8] containerization ensured every sprint concluded with a highly reproducible, runnable artifact.
\end{enumerate}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.15\textwidth}|p{0.3\textwidth}|p{0.45\textwidth}|}
        ...
        \hline
        Sprint 1 & Auth and Core Data & 5-Role User model, login/logout, CSRF, Core Academic Models. \\
        Sprint 2 & Student and Staff Affairs & Batch Excel uploads (pandas [16]), student filtering. \\
        Sprint 3 & Course Management & CourseOffering model, doctor assignments, lecture uploads. \\
        Sprint 4 & Attendance and Grading & Bulk attendance recording, grading templates. \\
        Sprint 5 & Quiz Engine & MCQ/Essay CRUD, auto-grading logic. \\
        Sprint 6 & Student Portal & Dashboards, schedules, grade views. \\
        Sprint 7 & Admin and Operations & Announcements, audit logging, deletion workflows. \\
        Sprint 8 & DevOps and Polish & Docker Compose, Nginx proxy, RTL (Arabic) UI support. \\
        Sprint 9 & Decentralized Workflows & Dean approvals, HOD assignments, Tuition constraints. \\
        Sprint 10 & Graduate Affairs & Career portal, graduation clearance, public certificate verification. \\
        \hline
    \end{tabular}
...
The following functional requirements are classified using IEEE 830-style [18] prioritization (Must / Should / Could). 
...
\begin{itemize}
    ...
    \item \textbf{FR-04 [Must]}: The system shall enforce isolated UI dashboards for all seven defined user roles.
    ...
    \item \textbf{FR-05b [Must]}: The system shall restrict access to quizzes and attendance views for students who have not paid their tuition.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{upload_preview.png}
    \caption{Student Affairs batch upload interface displaying the pre-flight validation preview, highlighting row-level data errors prior to database commit.}
    \label{fig:upload_preview}
\end{figure}

\subsection{Staff Affairs and Doctor Operations}
...
\subsection{Graduate Affairs Operations}

\begin{itemize}
    \item \textbf{FR-18 [Must]}: Graduate Affairs shall process multi-step graduation clearance checklists digitally.
    \item \textbf{FR-19 [Must]}: Graduate Affairs shall manage a career portal allowing companies to post job opportunities.
    \item \textbf{FR-20 [Must]}: The system shall provide a public verification endpoint for official graduation certificates.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{use_case_uml.png}
    \caption{UML Use Case Diagram mapping the eight primary system actors to their respective grouped use cases, illustrating the functional scope and permission boundaries of the portal.}
...
\begin{enumerate}
    ...
    \item System parses the file via pandas [16], validating headers and verifying that the file's payload matches the UI dropdown selections (preventing cross-department contamination).
    ...
    \item The system commits an \texttt{UploadHistory} audit log and returns a summary.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{seq_upload.png}
    \caption{UML Sequence Diagram detailing the Batch Student Upload flow, highlighting Pandas validation, iterative atomic database transactions, and asynchronous audit logging.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{admin_approval.png}
    \caption{Admin dashboard interface displaying the two-phase deletion approval workflow, highlighting pending deletion requests with justification reasons and explicit approve/reject actions.}
