...
\chapter{System and Architecture Design}
\label{chap:architecture}
\newpage
\section*{System and Architecture Design}
...
\begin{enumerate}
    \item \textbf{Presentation Tier}: A React~\cite{react}-based Single Page Application (SPA) deployed as a containerized service.
    \item \textbf{API Tier}: A Django REST Framework (DRF)~\cite{djangorest} backend, running inside a Gunicorn~\cite{gunicorn} WSGI server as a scalable AWS ECS Fargate task.
    \item \textbf{Data Tier}: A managed Amazon RDS for MySQL~\cite{mysql} 8.0 instance, providing high availability, automated backups, and automated patching.
    \item \textbf{Routing Tier}: An AWS Application Load Balancer (ALB) acting as the single entry point, routing \texttt{/api/*} requests to the backend service and serving the SPA for all other paths via an Nginx~\cite{nginx} container.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{aws_architecture_1.png}
    \caption{Cloud Deployment Architecture on AWS, illustrating the use of a Virtual Private Cloud (VPC) with public and private subnets to isolate ECS Fargate compute tasks, Amazon RDS database instance, and ElastiCache Redis cluster for session storage and caching.}
    \label{fig:four_tier_arch}
\end{figure}

\subsection{Architectural Trade-offs and Justifications}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.2\textwidth}|p{0.15\textwidth}|p{0.55\textwidth}|}
        ...
        \hline
        \textbf{Frontend} & React 19 [3] & Component-based UI, robust state handling. \\
        \textbf{UI Library} & Material UI (MUI) v7.3 [5] & Native RTL capability, Cairo font integration. \\
        \textbf{Backend} & Django 5.x [1] & Mature ORM, session management, reliable auth. \\
        \textbf{API Framework} & DRF 3.x [2] & Seamless serialization, extensible permission classes. \\
        \textbf{Database} & MySQL 8.0 & ACID compliance for strict academic records. \\
        \textbf{Infrastructure} & AWS ECS (Fargate) and RDS & Serverless container compute, zero-maintenance managed database. \\
        \textbf{Cache / Session} & Redis 7.x & In-memory session store and application cache. \\
        \hline
    \end{tabular}
...
\subsection{Sequence Diagram: Authentication and CSRF Handshake}
...
\begin{figure}[H]
    ...
    \caption{Sequence diagram detailing the decoupled CSRF handshake and session-based authentication flow between the React SPA and Django API.}
    \label{fig:seq_auth}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{batch_activity.png}
    \caption{Activity diagram illustrating the fault-tolerant batch processing pipeline. The iterative design ensures isolated failures do not abort the entire batch commit.}
    \label{fig:act_batch}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{quiz_state_machine.png}
    \caption{State machine diagram modeling the lifecycle of a student's quiz attempt, demonstrating the diverging paths for auto-graded MCQ assessments versus manually graded essay submissions.}
...
\begin{enumerate}
    \item \textbf{Identity and Access}: Manages user accounts and role-based permissions.
    \item \textbf{Academic Structure}: Defines the faculty's static hierarchy (Departments, Subjects, etc.).
    \item \textbf{Academic Operations}: Contains transactional data like grades, attendance, and tuition status.
    \item \textbf{Assessment}: The complete quiz engine schema and result publishing pipelines.
    \item \textbf{System and Communication}: Handles system-level features like audit logs and announcements.
    \item \textbf{Graduate Affairs and Career Services}: Manages graduation clearance, job postings, partner companies, and document requests.
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{erd_full.png}
    \caption{Entity-Relationship Diagram detailing the core domains of the system: Identity, Academic Structure, Operations, and Assessment. Demonstrates the central pivot role of the CourseOffering entity.}
    \label{fig:erd_full}
\end{figure}
...
\begin{table}[H]
    \centering
    \renewcommand{\arraystretch}{1.3}
    \small  % Smaller font for wide table
    \begin{tabular}{|p{0.18\textwidth}|p{0.42\textwidth}|p{0.3\textwidth}|}
        \hline
...
\subsection{Application Structure and Namespace}
...
|-- graduate_affairs/  # Graduation clearance, Career portal, Requests
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{backend_modules_1.png}
    \caption{Dependency Diagram showing the modular structure of the Django backend. Demonstrates how the academic, content, and system apps rely on the centralized users module for RBAC enforcement.}
...
MIDDLEWARE = [
    'corsheaders.middleware.CorsMiddleware',        # 1. Must be high for preflight checks (django-cors-headers)
    'django.middleware.security.SecurityMiddleware',
    'whitenoise.middleware.WhiteNoiseMiddleware',  # WhiteNoise~\cite{whitenoise} for static files
...
\begin{lstlisting}[language=Python, caption=AuditLog Model with Flexible JSONField (\texttt{academic/models.py})]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{react_tree.png}
    \caption{React Component Tree demonstrating the Context Provider nesting. The Theme and Auth providers wrap the Router, allowing the ProtectedRoute component to securely shield role-specific dashboards.}
...
\textbf{UI Feature Locking:} Beyond entire route protection, the frontend gracefully degrades the user experience by locking specific interactive features when constraints are not met. For example, if a student has unpaid tuition, the Quiz and Attendance buttons on their dashboard are grayed out and rendered with a lock icon, while a prominent alert explains the restriction.
...
\subsection{RTL-First Interface and Theming}

Because the faculty requires an Arabic-native interface, the UI was built RTL-first. It leverages Material-UI's [5] built-in RTL support, which uses the \texttt{stylis-plugin-rtl} library to automatically mirror CSS properties (e.g., converting \texttt{padding-left} to \texttt{padding-right}) when the theme's direction is set to \texttt{'rtl'}. The design incorporates the "Cairo" Google Font, which is heavily optimized for Arabic script legibility. Dark mode integration maps the university's primary color palette to low-luminance backgrounds to reduce eye strain.

\subsection{Build Optimization and Deployment}

The project utilizes multi-stage Docker [8] builds to create lean, production-optimized container images. The frontend build process, for example, uses a Node.js image to build the static React assets, then copies only the resulting \texttt{/dist} folder and the Nginx configuration into a lightweight \texttt{nginx:alpine} image for the final artifact. This ensures the production container has a minimal attack surface and does not contain any build-time dependencies.
...
The Nginx [10] container serves a dual purpose: it efficiently serves the static frontend assets and acts as a reverse proxy for the backend API. This single-entrypoint architecture simplifies CORS configuration and network security. The Nginx configuration is engineered to handle large file uploads and support client-side routing.

\begin{lstlisting}[float, language=nginx, caption=Nginx Reverse Proxy Configuration (\texttt{nginx.conf})]
server {
    listen 80;
