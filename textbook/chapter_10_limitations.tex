...
\begin{table}[htbp]
    ...
    \begin{tabular}{|c|p{3cm}|c|p{3cm}|p{5cm}|}
        ...
        \hline
        L-18 & \textbf{Manual deployment step} & Operations & After new image is built and pushed, admin must manually run \texttt{docker-compose~\cite{docker-compose} pull \&\& up -d} on production server. & Implement automated deployment webhook that triggers on image push to update production automatically. \\
        \hline
    \end{tabular}
    }
