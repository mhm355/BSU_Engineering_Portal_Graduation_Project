...
\begin{titlepage}
    ...
    {\large \textbf{Electronics and Communications Engineering}}\\[0.2cm]
    {\large at Beni-Suef University}\\[1.5cm]
...
Key system capabilities include hierarchical academic entity modeling (departments $\rightarrow$ specializations $\rightarrow$ levels $\rightarrow$ students), transaction-safe batch CSV/Excel import pipelines using Pandas for student enrollment and grade uploads, and a highly interactive multi-modal quiz engine. To guarantee data integrity and security, the system enforces the principle of least privilege across seven distinct user roles (Admin, Dean, HOD, Doctor, Student, Student Affairs, Staff Affairs) via custom Role-Based Access Control (RBAC) middleware, coupled with comprehensive audit logging for all administrative actions.
