...
The system implements a \textbf{five-role authorization model} (Admin, Student, Doctor, Student Affairs, Staff Affairs). Authorization is enforced at the backend through seven custom Django REST Framework (DRF) [2] permission classes. 
...
The project's GitHub Actions [15] deployment pipeline integrates \textbf{Trivy [14]} to execute static application security testing (SAST) on all generated Docker [8] artifacts. The pipeline is instructed to fail automatically (\texttt{exit-code: '1'}) if any CRITICAL or HIGH severity OS/library vulnerabilities are detected, preventing vulnerable code from reaching production.
