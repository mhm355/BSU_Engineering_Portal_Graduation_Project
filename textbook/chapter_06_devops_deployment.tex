...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{monitoring_architecture.png}
    \caption{Monitoring Architecture Diagram showing Prometheus scraping metrics from Node Exporter, MySQL Exporter, Redis Exporter, Nginx Exporter, and cAdvisor, with Alertmanager routing alerts to email and Slack.}
    \label{fig:monitoring-architecture}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{prometheus_targets.png}
    \caption{Prometheus Targets Page at \texttt{localhost:9090/targets} showing all exporter endpoints with health status.}
    \label{fig:prometheus-targets}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{grafana_dashboard.png}
    \caption{Grafana Dashboard showing system overview metrics including CPU usage, memory consumption, database connections, and application response times.}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{0.25\textwidth}|c|c|p{0.35\textwidth}|}
        ...
        \hline
        Frontend (Compute) & \textasciitilde\$10-20 & \textasciitilde\$5-10 & Fargate vs Container Apps (ACA) \\
        \hline
        Backend (Compute) & \textasciitilde\$10-20 & \textasciitilde\$5-10 & Fargate vs Container Apps (ACA) \\
        \hline
        Redis (Cache) & \textasciitilde\$5-10 & \textasciitilde\$5-10 & ElastiCache vs Container Apps (ACA) \\
        \hline
        Database (MySQL) & \textasciitilde\$15 & \textasciitilde\$15 & RDS (db.t3.micro) / Azure Database for MySQL (B1s) \\
        \hline
        Load Balancer & \textasciitilde\$20 & Included & ALB hourly charges vs native ACA ingress \\
        \hline
        \textbf{Total (Dev)} & \textasciitilde\$60-85 & \textasciitilde\$30-45 & Azure more cost-effective for this workload \\
        \hline
    \end{tabular}
\end{table}
...
\section{Enhancements and Recommendations}
