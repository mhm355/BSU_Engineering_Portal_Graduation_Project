...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{screenshot_login_flow.png}
    \caption{Authentication flow showing: (a) Login page with RTL form fields (Username / National ID, Password), and (b) First-Login Password Change page with validation rules displayed in Arabic.}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\textwidth]{screenshot_admin_dashboard.png}
    \caption{Admin Dashboard showing summary cards, RBAC-based navigation menu, and the audit log access point.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{screenshot_student_affairs_upload.png}
    \caption{Student Affairs Excel Upload showing: (a) department/level selection form, (b) file upload area with drag-and-drop, and (c) preview table with per-row validation status and Arabic error messages.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{screenshot_doctor_attendance.png}
    \caption{Doctor Bulk Attendance Recording interface showing date picker, student list with Present/Absent/Excused radio buttons, and automatic Excel export functionality.}
    \label{fig:bulk-attendance}
\end{figure}

\subsection{Student Portal: Grade Access}

The student-facing interface provides \textbf{24/7 self-service access} to academic records. This single screen replaces the previous workflow: student $\rightarrow$ Student Affairs office $\rightarrow$ wait days $\rightarrow$ check notice board.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{screenshot_student_grade_view.png}
    \caption{Student Grade View showing all enrolled subjects with complete grade breakdown per component (coursework, written, practical, attendance, quizzes, total).}
    \label{fig:student-grades}
\end{figure}

\textbf{Engineering Highlight---Real-Time Access:} Grades are visible the moment a doctor enters them (or when exam grades are approved by admin). Students see their exact breakdown per component, not just a final number.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{screenshot_quiz_interface.png}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{2.2cm}|p{3.2cm}|p{3.8cm}|p{2.8cm}|}
        ...
        \hline
        Unauthorized Access & Anyone with Excel has full access & Role-based---7 permission classes & \texttt{IsAdminRole}, \texttt{IsDoctorRole} \\
        \hline
