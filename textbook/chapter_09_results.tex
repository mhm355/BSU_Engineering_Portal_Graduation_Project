...
\newpage
\subsection{Dean, HOD and Graduate Affairs Dashboards [NEW FEATURES]}

Following the decentralized workflow enhancements, the Dean, Head of Department (HOD), and Graduate Affairs interfaces provide targeted capabilities:
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{graduate_affairs_dashboard.png}
        \caption{\small (b) Graduate Affairs Dashboard}
    \end{minipage}
    \caption{New role workflows: (a) Paginated analytics and statistics for the Dean, and (b) Interface for managing alumni clearance, certificates, and the career portal for Graduate Affairs.}
    \label{fig:dean-grad-workflows}
\end{figure}
...
\begin{table}[H]
    ...
    \begin{tabular}{|p{2.8cm}|p{4cm}|p{4cm}|c|}
        ...
        \hline
        Alumni Tracking and Services & Hard to reach alumni post-graduation. & Career portal, jobs, and digital requests. & \textbf{Continuous engagement} \\
        \hline
    \end{tabular}
    }
...
\begin{table}[H]
    ...
    \small
    \begin{tabular}{|p{2.3cm}|p{2.8cm}|p{3.5cm}|p{3.8cm}|}
        \hline
        \textbf{Dimension} & \textbf{Before} & \textbf{After} & \textbf{Evidence} \\
        \hline
        Duplicate Records & Same student in multiple Excel files & Impossible---\texttt{national\_id} is \texttt{unique=True} & \raggedright\texttt{User.national\_id} \newline \texttt{unique=True} \\
        \hline
        Grade Tampering & Anyone with file access could modify & Audit-logged with actor identity & \raggedright\texttt{AuditLog.action =} \newline \texttt{'GRADE\_UPDATED'} \\
        \hline
        Unauthorized Access & Anyone with Excel has full access & Role-based---10 permission classes & \raggedright\texttt{IsAdminRole}, \newline \texttt{IsDeanRole} \\
        \hline
        Data Loss & High risk---paper, local files & Docker volumes + version-controlled config & \raggedright\texttt{volumes:} \newline \texttt{db\_data:/var/lib/mysql} \\
        \hline
        Attendance Accuracy & Paper signatures---illegible, lost & Digital with PRESENT / ABSENT / EXCUSED status & \raggedright\texttt{Attendance.} \newline \texttt{unique\_together} \\
        \hline
