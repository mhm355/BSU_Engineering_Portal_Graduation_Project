...
\begin{table}[H]
    ...
    \small
    \begin{tabular}{|p{2.2cm}|p{2.8cm}|p{4.5cm}|p{4cm}|}
        \hline
        \textbf{Dimension} & \textbf{Before} & \textbf{After} & \textbf{Evidence} \\
        \hline
        Duplicate Records & Same student in multiple Excel files & Impossible---\texttt{national\_id} is \texttt{unique=True} & \texttt{User.national\_id}, \newline \texttt{unique=True} \\
        \hline
        Grade Tampering & Anyone with file access could modify & Audit-logged with actor identity & \texttt{AuditLog.action} \newline \texttt{GRADE\_UPDATED} \\
        \hline
        Unauthorized Access & Anyone with Excel has full access & Role-based---10 permission classes & \texttt{IsAdminRole}, \newline \texttt{IsDeanRole} \\
        \hline
        Data Loss & High risk---paper, local files & Docker volumes + version-controlled config & \texttt{db\_data:} \newline \texttt{/var/lib/mysql} \\
        \hline
        Attendance Accuracy & Paper signatures---illegible, lost & Digital with PRESENT / ABSENT / EXCUSED status & \texttt{Attendance.} \newline \texttt{unique\_together} \\
        \hline
