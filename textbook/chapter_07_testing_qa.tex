...
\subsection{Decentralized Approval Pipeline Testing}

To verify the integrity of the Dean approval workflow, integration tests simulate the multi-actor state transitions required to publish grades.

\begin{lstlisting}[language=Python, caption=Grade Approval Integration Test]
def test_dean_approval_workflow(self):
    # 1. Doctor enters grades (Status: PENDING)
    self.client.post('/api/academic/grades/submit/', {'course': self.course.id})
    self.assertEqual(ExamGrade.objects.get(course=self.course).status, 'PENDING')
    
    # 2. Dean approves grades (Status: APPROVED)
    self.client.force_authenticate(user=self.dean_user)
    self.client.post(f'/api/academic/grades/{self.course.id}/approve/')
    
    # 3. Student views grades (Must be visible now)
    self.client.force_authenticate(user=self.student_user)
    response = self.client.get('/api/academic/grades/my_grades/')
    self.assertIn(self.course.subject.name, response.data)
\end{lstlisting}
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|p{2.8cm}|p{2.5cm}|p{3cm}|c|}
        ...
        \hline
        TC-DN01 & Dean Approval & Click "Approve" & Status $\rightarrow$ APPROVED & \textcolor{green!80!black}{\ding{51}} Passed \\
        \hline
